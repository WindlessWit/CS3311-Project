...
function renderJobList(listEl, filters = {}) {
    ...
    filtered.forEach((job) => {
        ...
        form.className = "job-assign-form";
        form.dataset.jobId = job.id;
        const hasEmployees = employeeDataCache.length > 0;
        form.innerHTML = `
            <label class="form-label">Assign employee(s) to this job</label>
            <div class="form-grid">
                <div class="field">
                    <select multiple data-job-employee-select aria-label="Select employees for ${job.project}">
                        <option value="" disabled ${hasEmployees ? "" : "selected"}>
                            ${hasEmployees ? "Select one or more employees" : "Loading employees…"}
                        </option>
                        ${employeeDataCache
                            .map(
                                (employee) =>
                                    `<option value="${employee.id}" data-email="${employee.email}" data-name="${employee.name}">${employee.name} — ${employee.role}</option>`
                            )
                            .join("")}
                    </select>
                    <p class="form-help">Hold Ctrl/Cmd to select multiple people.</p>
                </div>
                <div class="field">
                    <input type="date" data-job-start value="${job.startDate}" min="${getTodayISO()}" aria-label="Start date for ${job.project}">
                    <input type="text" data-job-note placeholder="Add a quick note (optional)" aria-label="Note to dispatch">
                </div>
            </div>
            <div class="form-actions">
                <button class="btn tertiary" type="button" data-assign-job data-job-id="${job.id}" ${
                    hasEmployees ? "" : "disabled"
                }>Assign selected employees</button>
                <span class="pill small" data-job-feedback>${hasEmployees ? "" : "Load roster to begin"}</span>
            </div>
        `;
...
function initAssignments() {
    ...
    jobListEl?.addEventListener("click", async (event) => {
        const button = event.target.closest("[data-assign-job]");
        if (!button) return;

        const jobId = button.dataset.jobId;
        const job = jobDataCache.find((item) => item.id === jobId);
        const row = button.closest("li");
        const select = row?.querySelector("[data-job-employee-select]");
        const startInput = row?.querySelector("[data-job-start]");
        const noteInput = row?.querySelector("[data-job-note]");
        const feedback = row?.querySelector("[data-job-feedback]");
        ...
        if (!selected.length) {
            if (feedback) {
                ...
                feedback.className = "pill small error";
            }
            return;
        }
        ...
        if (feedback) {
            ...
            feedback.className = "pill small";
        }

        const startDate = startInput?.value || job.startDate || getTodayISO();
        const notes = noteInput?.value?.trim() || "Assigned via job list";
        let successCount = 0;

        for (const employeeOption of selected) {
            const formData = new FormData();
            formData.set("employeeId", employeeOption.value);
            formData.set("name", employeeOption.dataset.name || "");
            formData.set("email", employeeOption.dataset.email || "");
            formData.set("jobId", job.id);
            formData.set("shift", job.shift);
            formData.set("startDate", startDate);
            formData.set("notes", notes);

            const success = await submitAssignment(formData, feedback);
            const submission = {
                employeeId: employeeOption.value,
                name: employeeOption.dataset.name || "",
                email: employeeOption.dataset.email || "",
                jobId: job.id,
                shift: job.shift,
                startDate,
                notes,
        ...
        if (feedback) {
            feedback.textContent = successCount
                ? `Sent ${successCount} assignment${successCount > 1 ? "s" : ""}`
                : "Saved locally while offline.";
            feedback.className = successCount ? "pill small success" : "pill small error";
        }
    });
